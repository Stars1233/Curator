...
| `config["resource_wait_timeout_s"]` | `float` | `5.0` | Maximum seconds to wait for stage resources; must be non-negative |
| `config["resource_wait_interval_s"]` | `float` | `0.2` | Seconds between resource availability checks; must be positive |

Before creating each actor pool, the executor waits for the stage's requested resources. If they are not available within `resource_wait_timeout_s`, execution raises `TimeoutError`. Increase this timeout when a previous stage may take longer to release resources.
