...
- Access to the gated default model: accept the [EmbeddingGemma usage license](https://huggingface.co/google/embeddinggemma-300m), then authenticate with a Hugging Face read token through `HF_TOKEN` or the `hf_token` parameter
...
| `hf_token` | str \| None | `None` | Hugging Face read token for private or gated models |
...
- **Parquet:** Selects as many complete files as fit the actor's live GPU-memory budget. Only one
  contiguous float32 embedding array remains resident during fitting; each input frame is released
  after its embeddings are copied into that array.
...
- **Parquet:** Exact footer counts size one contiguous float32 fit array. cuDF reads the sampled files
  in chunks that share the remaining GPU memory between output and decompression. The sampled frames
  are released during the first pass; the second pass rereads all files to predict and write every row.
